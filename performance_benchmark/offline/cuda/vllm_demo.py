...
def run_qwen2_5_vl(args):
    ...
    if args.multi_batch:
        ...
        for batch_size in batch_sizes:
            test_images = min(batch_size, total_images)
            print(f"\nTesting batch size: {batch_size}, using first {test_images} images")
            throughput = batch_inference(model, sampling_params, text, images, batch_size)
            images_per_second = throughput
            iterations_per_second = throughput / batch_size
            print(f"batch {batch_size} throughput: {iterations_per_second:.4f} it/s, {images_per_second:.2f} images/s")
    else:
        # Process all images with specified batch size
